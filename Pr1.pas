...
begin
  If OpenPictureDialog1.Execute then
    begin
      ...
      Label1.Caption := 'ƒоступно дл€ записи:: ' + IntToStr((Bmp.Width * Bmp.Height * 2) div 8) + 'байт';
    end;
end;
...
begin
  ...
  Buf := 'Walde';     {записываем свою подпись, подтверждающую то, что мы записывали информацию в данную картинку}
  ...
  MS1 := TMemoryStream.Create;//создаем поток
  BufI := MS.Size;
  MS1.Write(BufI, 4);
  MS1.Position := 0;
  MS.Position := 0;
...
begin
  ...
  If Buf <> 'Walde' then
    begin
      Messagedlg('Ќичего не записанно', mtConfirmation, [mbOK], 0);
      Exit;
    end;
